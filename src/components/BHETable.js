...
export default function BHETable({  // 
    bheState,                          
    setBheState,
    }){
    ...
    //standard income frequencies
        ...
        // Event handlers 
            ...
            const handleRowChange = (index, field, userInput) => {
              ...
              const toggleDropdown = (index) => {
                ...
                setTimeout(() => {
                  console.log("State after toggle:", bheState.DropdownOpen);
                }, 0);
              };
            ...
            const handleFreqChange = (index, field, newFreq) => {
                ...
                setTimeout(() => {
                  console.log("Updated pteData:", bheState.Data);
                }, 0);
            };
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {bheState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={bheState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={bheState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {bheState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
