...
/* This component displays a table which promts the user to enter their  Child Costs > Family and Pets >Expenditure Items */
    ...
    //standard income frequencies
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {cfeState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={cfeState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={cfeState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {cfeState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
        ...
        return (
            <>
                <h4>Other Child Costs</h4>
                <StyledTable borderless>
                    {renderTableHeaders()}
                    {renderTableRows()}
                </StyledTable>
                <StyledButton onClick={addRow}>
                    Add Other Child Expenditure Item
                </StyledButton>
            </>
        );
}
