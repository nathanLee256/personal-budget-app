...
/* This component displays a table which promts the user to enter their  Debts > Debt Repayments >Expenditure Items 
 LLETable is used as the component name instead of OLETable to avoid naming conflict with Other>Living Costs> Expenditure*/

export default function DDETable({  // 
    ddeState,                          
    setDdeState,
    }){
    ...
    //standard income frequencies
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {ddeState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={ddeState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={ddeState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {ddeState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
