...
/* This component displays a table which promts the user to enter their  General Living Costs >Living >Expenditure Items */
export default function GLETable({  // 
    gleState,                          
    setGleState,
    }){
    ...
    //standard income frequencies
        ...
        // Event handlers 
            ...
            const handleRowChange = (index, field, userInput) => {
              ...
              const toggleDropdown = (index) => {
                ...
                setTimeout(() => {
                  console.log("State after toggle:", gleState.DropdownOpen);
                }, 0);
              };
            ...
            const handleFreqChange = (index, field, newFreq) => {
                ...
                setTimeout(() => {
                  console.log("Updated pteData:", gleState.Data);
                }, 0);
            };
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {gleState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={gleState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={gleState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {gleState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
        ...
        return (
            <>
                ...
                <StyledButton onClick={addRow}>
                    Add General Living Expenditure Item
                </StyledButton>
            </>
        );
}
