...
/* This component displays a table which promts the user to enter their  Health-Related Living Costs >Living >Expenditure Items */
export default function HLETable({  // 
    hleState,                          
    setHleState,
    }){
    ...
    //standard income frequencies
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {hleState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={hleState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={hleState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {hleState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
        ...
        return (
            <>
                <h4>Health Related Living Costs</h4>
                <StyledTable borderless>
                    {renderTableHeaders()}
                    {renderTableRows()}
                </StyledTable>
                <StyledButton onClick={addRow}>
                    Add Health Expenditure Item
                </StyledButton>
            </>
        );
}
