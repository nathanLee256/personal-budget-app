...
/* This component displays a table which promts the user to enter their  Investment > Future Needs >Expenditure Items 
 LLETable is used as the component name instead of OLETable to avoid naming conflict with Other>Living Costs> Expenditure*/

export default function IFETable({  // 
    ifeState,                          
    setIfeState,
    }){
    ...
    //standard income frequencies
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {ifeState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={ifeState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={ifeState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {ifeState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
        ...
        return (
            <>
                ...
                <StyledButton onClick={addRow}>
                    Add Investment Expenditure Item
                </StyledButton>
            </>
        );
}
