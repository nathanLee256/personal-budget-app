...
/* This component displays a table which promts the user to enter their  Other Leisure > Leisure >Expenditure Items 
 LLETable is used as the component name instead of OLETable to avoid naming conflict with Other>Living Costs> Expenditure*/

export default function LLETable({  // 
    lleState,                          
    setLleState,
    }){
    ...
    //standard income frequencies
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {lleState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={lleState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={lleState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {lleState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
        ...
        return (
            <>
                ...
                <StyledButton onClick={addRow}>
                    Add Other Leisure Expenditure Item
                </StyledButton>
            </>
        );
}
