...
/* This component displays a table which promts the user to enter their  Other Income> Income Items */
    ...
    //standard income frequencies
        ...
        // Event handlers 
            ...
            const handleRowChange = (index, field, userInput) => {
              ...
              const toggleDropdown = (index) => {
                ...
                setTimeout(() => {
                  console.log("State after toggle:", oiState.DropdownOpen);
                }, 0);
              };
            ...
            const handleFreqChange = (index, field, newFreq) => {
                ...
                setTimeout(() => {
                  console.log("Updated pteData:", oiState.Data);
                }, 0);
            };
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {oiState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={oiState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={oiState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {oiState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
