...
export default function PTETable({  // 
    pteState,                 // The state array for income rows          
    setPteState,
    }){
    ...
    //standard income frequencies
        ...
        // Event handlers for SI table
            ...
            const addPteRow = () => {
                setPteState((prevState) => ({
                  ...
                  ptePlaceholders: [
                    ...prevState.ptePlaceholders,
                    "e.g. other item", // Add a new placeholder
                  ],
                  pteDropdownOpen: [...prevState.pteDropdownOpen, false], // Add a new dropdown state
                }));
              };
            ...
            const handlePteRowChange = (index, field, userInput) => {
                setPteState((prevState) => ({
                  ...prevState, // Preserve other properties of the state
                  pteData: prevState.pteData.map((row, rowIndex) =>
              ...
              const toggleDropdown = (index) => {
                setPteState((prevState) => ({
                  ...prevState, // Preserve other properties of the state
                  pteDropdownOpen: prevState.pteDropdownOpen.map((isOpen, i) =>
                ...
                setTimeout(() => {
                  console.log("State after toggle:", pteState.pteDropdownOpen);
                }, 0);
              };
            ...
            const handleFreqChange = (index, field, newFreq) => {
                ...
                setTimeout(() => {
                  console.log("Updated pteData:", pteState.pteData);
                }, 0);
            };
            ...
            const handleDelete = (indexToDelete) => {
                setPteState((prevState) => ({
                  ...prevState, // Preserve other properties of the state
                  pteData: prevState.pteData.filter((_, index) => index !== indexToDelete),
                  ptePlaceholders: prevState.ptePlaceholders.filter((_, index) => index !== indexToDelete),
                  pteDropdownOpen: prevState.pteDropdownOpen.filter((_, index) => index !== indexToDelete),
                }));
            };
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderPTETableRows = () => (
                <tbody>
                    {pteState.pteData.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handlePteRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={pteState.ptePlaceholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                                    ...
                                    <Input
                                        value={row.amount}
                                        onChange={(e) => handlePteRowChange(rowIndex, "amount", e.target.value)}
                            ...
                            <td>
                                <Dropdown
                                isOpen={pteState.pteDropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {pteState.pteData[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
        ...
        return (
            <>
                ...
                <StyledButton onClick={addPteRow}>
                    Add Public Transport Travel Item
                </StyledButton>
            </>
        );
}
