...
/* This component displays a table which promts the user to enter their  Rent/Mortgage> Home > Expenditure Items */
export default function RHETable({  // 
    rheState,                          
    setRheState,
    }){
    ...
    //standard income frequencies
        ...
        // Event handlers 
            ...
            const handleRowChange = (index, field, userInput) => {
              ...
              const toggleDropdown = (index) => {
                ...
                setTimeout(() => {
                  console.log("State after toggle:", rheState.DropdownOpen);
                }, 0);
              };
            ...
            const handleFreqChange = (index, field, newFreq) => {
                ...
                setTimeout(() => {
                  console.log("Updated pteData:", rheState.Data);
                }, 0);
            };
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderTableRows = () => (
                <tbody>
                    {rheState.Data.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={rheState.Placeholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                            ...
                            <td>
                                <Dropdown
                                isOpen={rheState.DropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {rheState.Data[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
        ...
        return (
            <>
                <h4>Rent and Mortgage</h4>
                <StyledTable borderless>
                    {renderTableHeaders()}
                    {renderTableRows()}
                </StyledTable>
                <StyledButton onClick={addRow}>
                    Add Rent/Mortgage Expenditure Item
                </StyledButton>
            </>
        );
}
