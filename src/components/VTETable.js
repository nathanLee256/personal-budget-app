...
export default function VTETable({  // 
    vteState,                 // The state array for income rows          
    setVteState,
    }){
    ...
    //standard income frequencies
        ...
        // Event handlers for SI table
            ...
            const addVteRow = () => {
                setVteState((prevState) => ({
                  ...
                  vtePlaceholders: [
                    ...prevState.vtePlaceholders,
                    "e.g. other item", // Add a new placeholder
                  ],
                  vteDropdownOpen: [...prevState.vteDropdownOpen, false], // Add a new dropdown state
                }));
              };
            ...
            const handleVteRowChange = (index, field, userInput) => {
                setVteState((prevState) => ({
                  ...prevState, // Preserve other properties of the state
                  vteData: prevState.vteData.map((row, rowIndex) =>
              ...
              const toggleDropdown = (index) => {
                setVteState((prevState) => ({
                  ...prevState, // Preserve other properties of the state
                  vteDropdownOpen: prevState.vteDropdownOpen.map((isOpen, i) =>
                ...
                setTimeout(() => {
                  console.log("State after toggle:", vteState.vteDropdownOpen);
                }, 0);
              };
            ...
            const handleFreqChange = (index, field, newFreq) => {
                ...
                setTimeout(() => {
                  console.log("Updated vteData:", vteState.vteData);
                }, 0);
            };
            ...
            const handleDelete = (indexToDelete) => {
                setVteState((prevState) => ({
                  ...prevState, // Preserve other properties of the state
                  vteData: prevState.vteData.filter((_, index) => index !== indexToDelete),
                  vtePlaceholders: prevState.vtePlaceholders.filter((_, index) => index !== indexToDelete),
                  vteDropdownOpen: prevState.vteDropdownOpen.filter((_, index) => index !== indexToDelete),
                }));
            };
        ...
        // used in the components which will render the other tables. These functions return JSX themselves 
            ...
            const renderVTETableRows = () => (
                <tbody>
                    {vteState.vteData.map((row, rowIndex) => (
                        <tr key={rowIndex}>
                            <td>
                                <Input
                                    value={row.item}
                                    onChange={(e) => handleVteRowChange(rowIndex, "item", e.target.value)}
                                    placeholder={vteState.vtePlaceholders[rowIndex]}
                                />
                            </td>
                            <td>
                                <InputGroup>
                                    ...
                                    <Input
                                        value={row.amount}
                                        onChange={(e) => handleVteRowChange(rowIndex, "amount", e.target.value)}
                            ...
                            <td>
                                <Dropdown
                                isOpen={vteState.vteDropdownOpen[rowIndex]} // Controls visibility based on state
                                toggle={() => toggleDropdown(rowIndex)} // Toggles visibility
                                >
                                <DropdownToggle caret color="info">
                                    {vteState.vteData[rowIndex].frequency || "Select Frequency"}
                                </DropdownToggle>
                                <DropdownMenu>
                                    {
        ...
        return (
            <>
                ...
                <StyledButton onClick={addVteRow}>
                    Add Vehicle Travel Item
                </StyledButton>
            </>
        );
}
